...
export default class APIHandler {
  ...
  async loadInitialData(pagina = 1) {
    ...
    try {
      const response = await fetch(
        `${this.apiUrl}?limit=${initialLoadAmount}&page=${pagina}`,
        options,
      )
      if (!response.ok) {
        throw new Error(
  ...
  async filterData(pagina) {
    ...
    try {
      const response = await fetch(
        `${this.apiUrl}?limit=${itemsPerPage}&page=${pagina}`,
        options,
      )
  ...
  async fetchPut(url, datosActualizar) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error(
          "Error al actualizar el campo url_img en el servidor:",
          response.statusText,
        )
        return false
      }
  ...
  async uploadMultiple(uploadUrl, formData) {
    ...
    try {
      ...
      if (!uploadResponse.ok) {
        ...
        throw new Error(
          `Error al subir el archivo: ${errorMessage || uploadResponse.statusText}`,
        )
      }
  ...
  async enviarDatosEspecificos(datos, endpointEspecifico) {
    ...
    try {
      ...
      throw new Error(
        "Hubo un problema al enviar los datos al endpoint específico",
      )
    }
  }

  async recaptchaValidation(token) {
    const url = "https://exclusivosportada.com/src/verify-recaptcha.php"
