<template>
	<div>
		...
		<h3> Formulario Servicios Públicos </h3>
	</div>
			...
			<table class="table table-bordered">
					...
					<tr>
						<th colspan="4">
							<div class="icons fecha_hora_span">
								...
								<div>{{ public_services.register_date }}</div>
							</div>
						</th>
					</tr>
				...
				<tbody>
					...
					<tr>
						...
						<td v-if="public_services.id_status">{{ public_services.id_status.status }}</td>
						<th class="th_row_icon_spn">
							<div class="icons">
					...
					<tr>
						...
						<td v-if="public_services.id_customer">{{ public_services.id_customer.fullname }}</td>
						...
						<td v-if="public_services.id_customer">{{ public_services.id_customer.celphone }}</td>
					</tr>
					<tr>
						<th class="th_row_icon_spn">
							<div class="icons">
						...
						<td v-if="public_services.id_customer">
							{{ public_services.id_customer.document_type }} - {{ public_services.id_customer.document }}
						</td>
						<th class="th_row_icon_spn">
							<div class="icons">
								<iconsSvg email="true" />
						...
						<td v-if="public_services.id_customer">{{ public_services.id_customer.email }}</td>
					</tr>
					<tr>
						...
						<td colspan="3">{{ public_services.description }}</td>
					</tr>
...
const created = async () => {
  try {
    ...
    const res = await public_services_apiHandler.cargarDatos(id, public_services_base_endpoint + actions.getBy);
...
const openPDFView = () => {
  const htmlContent = generateHTMLContent(public_services);
