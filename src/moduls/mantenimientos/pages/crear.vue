    ...
    <div class="card">
      ...
      <div class="card-body my-2">
        <form v-on:submit.prevent="crearMantenimiento">
          ...
          <selectS
            inputId="'floatingSelect'"
            :selected-value="mantenimiento.headquarter"
            :label="'Reportar a *'"
            :options="reportOptions"
            @update:select-value="mantenimiento.headquarter = $event"
            :editable="!loading"
          /><br />
          <selectS
            inputId="'floatingSelect'"
            :selected-value="mantenimiento.document_type"
            :label="'Tipo de Identificación *'"
            :options="docOptions"
            @update:select-value="mantenimiento.document_type = $event"
            :editable="true"
          />
          ...
          <div class="my-3">
            <InputText
              type="text"
              inputId="floatingInputGroup1"
              v-model="mantenimiento.document"
          ...
          <div class="my-3">
            <InputText
              type="text"
              inputId="floatingInputGroup1"
              v-model="mantenimiento.fullname"
              placeholder="nombre"
          ...
          <div class="form-group">
            <CountrySelector
              class="input-field2"
              type="text"
              inputId="floatingInputGroup2"
              v-model="mantenimiento.cellphoneExt"
              placeholder="extencion"
              label="Pais o Indicativo"
              errormessage="Este campo es obligatorio*"
              :max="10"
              :autocomplete="true"
            />
            <InputText
              ...
              type="text"
              inputId="floatingInputGroup2"
              v-model="mantenimiento.cellphone"
          ...
          <div class="my-3">
            <InputText
              type="text"
              inputId="floatingInputGroup1"
              v-model="mantenimiento.address_maintenance"
          ...
          <div class="my-3">
            <selectS
              inputId="'floatingSelect'"
              :selected-value="mantenimiento.category"
              :label="'Categoría *'"
              :options="categoriaOptions"
              @update:select-value="mantenimiento.category = $event"
              :editable="!loading"
            />
          </div>
          ...
          <div class="container">
            <ContainerText
              :textValue="mantenimiento.description"
              inputId="textArea"
              label="Descripción"
              :rows="3"
              :maxlength="180"
          ...
          <div class="mb-3">
            <label
              ...
              >Reglamento General de Protección de Datos *</label
            >
...
} from "../APIHandler.mantenimientos"
...
onMounted(async () => {
  try {
    await Promise.all([
      ...
      fetchDataFromAPI(
        category_base_endpoint + actions.getAll,
        categoriaOptions,
      ),
    ])
  } catch (error) {
    ...
    console.error(
      "No se encontraron datos iniciales. Por favor, inténtalo de nuevo más tarde.",
    )
  }
})
...
const crearcustomer = async () => {
  try {
    ...
    if (clienteExistente) {
      ...
      const customerCreado = await maintenance_apiHandler.fetchPost(
        datoscustomer,
        customer_base_endpoint + "/" + actions.create,
      )
    ...
    alert(
      "Hubo un problema al crear o buscar el cliente. Por favor, inténtalo de nuevo más tarde.",
    )
    throw error
  }
}
const enviarCorreoDesdeCreated = async (idMantenimiento) => {
  try {
    ...
    const res = await maintenance_apiHandler.cargarDatos(
      idMantenimiento,
      maintenance_base_endpoint + actions.getBy,
    )
    ...
    if (res.register_date) {
      ...
      console.warn(
        "Los datos obtenidos de la API no contienen la fecha esperada.",
      )
    }
    ...
    if (mantenimiento.value && mantenimiento.value.email) {
      ...
      const response = await maintenance_apiHandler.enviarDatosEspecificos(
        datosAEnviar,
        endpointEmail,
      )
      ...
      console.error(
        "Datos necesarios para el correo electrónico no disponibles.",
      )
    }
  } catch (error) {
    console.error(
      "Error al obtener o procesar los datos o al enviar el correo electrónico:",
      error,
    )
  }
}
...
const crearMantenimiento = async () => {
  try {
    ...
    alert(
      "Hubo un problema al crear el mantenimiento. Por favor, inténtalo de nuevo más tarde.",
    )
    throw error
  } finally {
...
const handleFileChange = async (idMantenimiento) => {
  try {
    if (files) {
      ...
      const uploadResponse = await maintenance_apiHandler.uploadMultiple(
        uploadUrl,
        formData,
      )
      if (uploadResponse) {
        ...
        const updateResponse = await maintenance_apiHandler.fetchPut(
          updateUrl,
          { url_img: uploadResponse },
        )
